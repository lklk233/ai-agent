...
**已融合开源项目 [Paper-Agent](https://github.com/Tswoen/Paper-Agent) 的多智能体文献调研工作流**（检索规划 → 摘要阅读 → 分层分析 → 大纲生成 → 综述写作），保留其五 Agent 设计并本地化：检索源改为本地语料库，LLM 全部走本地 Ollama，保持零框架轻量化。
...
- 单用户本地使用（默认绑定 127.0.0.1）
- 所有命令使用虚拟环境中的 `python`（本项目在 conda 环境 `localdocument_comb` 中开发）
- v1 范围：A 组核心功能；B 组（考古实体抽取/时间线/收藏夹）规划于二期
